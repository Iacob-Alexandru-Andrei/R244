...
from .server import BoServer
from flwr.server.strategy import FedAvg, Strategy

DEFAULT_SERVER_ADDRESS = "[::]:8080"


def start_bo_server(  # pylint: disable=too-many-arguments
    server_address: str = DEFAULT_SERVER_ADDRESS,
    server: Optional[BoServer] = None,
    ...
    # Start gRPC server
    grpc_server = flwr.server.grpc_server.grpc_server(
...
def _init_defaults(
    server: Optional[BoServer],
    config: Optional[Dict[str, int]],
    strategy: Optional[Strategy],
) -> Tuple[BoServer, Dict[str, int]]:
    # Create server instance if none was given
    if server is None:
        client_manager = SimpleClientManager()
        if strategy is None:
            strategy = FedAvg()
        server = BoServer(client_manager=client_manager, strategy=strategy)
...
def _fl(
    server: BoServer, config: Dict[str, int], force_final_distributed_eval: bool
) -> History:
    # Fit model
